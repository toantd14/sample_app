<form id="form-edit" action="{{ route('parking-spaces.update', ['parking_space' => $slotParking->serial_no]) }}"
    role="form">
    @csrf
    ...
    <div class="form-group row">
        ...
        <div class="col-8 col-md-10">
            <input type="checkbox" class="check-type" {{ $slotParking->kbn_standard == 1 ? 'checked' : '' }}
                name="car_type[kbn_standard]" value="1"><label class="mr-3 ml-1">普通</label>
            <input type="checkbox" class="check-type" {{ $slotParking->kbn_3no == 1 ? 'checked' : '' }}
                name="car_type[kbn_3no]" value="1"><label class="mr-3 ml-1">３ナンバー</label>
            <input type="checkbox" class="check-type" {{ $slotParking->kbn_lightcar == 1 ? 'checked' : '' }}
                name="car_type[kbn_lightcar]" value="1"><label class="mr-3 ml-1">軽自動車</label>

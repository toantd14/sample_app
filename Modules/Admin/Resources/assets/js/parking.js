...
$('select[name="owner_cd"]').on('change', function () {
    if (this.value) {
        $.ajax({
            type: 'get',
            dataType: 'json',
            url: urlGetFlgOwner + '/' + this.value,
            success: function (response) {
                $("input:radio[name=mgn_flg]").removeAttr('disabled');
                $("input:radio[name=mgn_flg][value='" + response.mgn_flg + "']").prop('checked', true);
            },
            error: function (response) {
                $("input:radio[name=mgn_flg]").attr('disabled', 'disabled');
            }
        })
    }
});

@php
use \App\Models\Owner;
@endphp

@extends('owner::layouts.master')

@section('title', '会員情報編集')

@section('css')
<link rel="stylesheet" href="{{ mix('/css/validate_register.css') }}">
<link rel="stylesheet" href="{{ mix('/css/edit-owner.css') }}">
<link rel="stylesheet" href="{{ mix('/css/admin/register-parking.css') }}">
@endsection

@section('content')
<div class="card-warning">
    <div class="card-body pl-0 pr-0">
        <form id="form-owner-edit" method="POST" action="{{ route('member.update', $owner->owner_cd) }}"
            enctype="multipart/form-data" class="form-check-code">
            <input type="hidden" name="_method" value="PUT">
            @csrf
            <div class="form-group row d-none d-md-flex">
                <div class="offset-sm-6 col-sm-6">
                    <a class="btn btn-primary" href="{{ route('bank-info.show', $owner->owner_cd) }}">口座情報</a>
                    <a class="btn btn-primary" href="{{ route('change-password.edit', $owner->owner_cd) }}">パスワード変種</a>
                </div>
            </div>
            @if(session('success'))
            @include('owner::layouts.modal_success')
            @endif
            @if (session()->has('editErrors'))
            <div class="alert alert-danger">{{ session()->get('editErrors') }}</div>
            @endif
            <div class="form-group row">
                <label class="col-md-2 col-4 label-text font-weight-normal">区分　<b class="text-danger">*</b></label>
                <div class="col-md-10 col-8">
                    @if ($owner->kubun == Owner::PERSONAL)
                    <input class="role" type="radio" name="kubun" value="0" checked><label class="mr-3 ml-1">個人</label>
                    <input class="role" id="company" name="kubun" value="1" type="radio"><label
                        class="mr-3 ml-1">法人</label>
                    @else
                    <input class="role" type="radio" name="kubun" value="0"><label class="mr-3 ml-1">個人</label>
                    <input class="role" type="radio" id="company" name="kubun" value="1" checked><label
                        class="mr-3 ml-1">法人</label>
                    @endif
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4 label-text font-weight-normal">お名前 <b class="text-danger">*</b></label>
                <div class="col-md-6 col-8">
                    <input type="text" name="name_c" value="{{ old('name_c') ?? $owner->name_c }}" class="form-control">
                    @if($errors->any() && $errors->has('name_c'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('name_c') }}</p>
                    @endif
                </div>
                <div class="div-hident col-4"></div>
                <div id="text-guide" class="col-md-4 col-8 mt-2">
                    （法人の場合、法人名を入力して下さい）
                </div>
            </div>
            <div id="info-company">
                <div class="form-group row">
                    <label class="col-md-2 label-text col-4 font-weight-normal">担当者名 <b class="text-danger">*</b></label>
                    <div class="col-md-5 col-8">
                        <input type="text" name="person_man" value="{{ old('person_man') ?? $owner->person_man }}"
                            class="form-control">
                        @if($errors->any() && $errors->has('person_man'))
                        <p class="text-danger error mb-2 mt-2">{{ $errors->first('person_man') }}</p>
                        @endif
                    </div>
                    <div class="div-hident col-4"></div>
                    <div class="col-md-4 col-8 mt-2">
                        （法人の連絡ご担当者名を入力して下さい）
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-md-2 col-4 label-text font-weight-normal">部署名</label>
                    <div class="col-md-5 col-8">
                        <input type="text" name="department" value="{{ old('department') ?? $owner->department }}"
                            class="form-control">
                        @if($errors->any() && $errors->has('department'))
                        <p class="text-danger error mb-2 mt-2">{{ $errors->first('department') }}</p>
                        @endif
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-md-2 col-4 label-text font-weight-normal">法人HP-URL</label>
                    <div class="col-md-6 col-8">
                        <input type="text" name="hp_url" value="{{ old('hp_url') ?? $owner->hp_url }}"
                            class="form-control">
                        @if($errors->any() && $errors->has('hp_url'))
                        <p class="text-danger error mb-2 mt-2">{{ $errors->first('hp_url') }}</p>
                        @endif
                    </div>
                    <div class="div-hident col-4"></div>
                    <div class="col-md-4 col-8 mt-2">
                        （ホームページのURLを入力して下さい）
                    </div>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4 label-text font-weight-normal">メールアドレス <b class="text-danger">*</b></label>
                <div class="col-md-6 col-8">
                    <input type="text" name="mail_add" value="{{ old('mail_add') ?? $owner->mail_add }}"
                        class="form-control">
                    @if($errors->has('mail_add') || session()->has('emailExits'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('mail_add') }} {{ session()->get('emailExits') }}</p>
                    @endif
                </div>
                <div class="div-hident col-4"></div>
                <div class="col-md-4 col-8 mt-2">
                    （ログインIDとなります。）
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4 label-text font-weight-normal">住所 <b class="text-danger">*</b><span
                        class="float-right">〒</span></label>
                <div class="col-md-5 col-8">
                    <input type="text" name="zip_cd"
                        value="{{ old('zip_cd') ?? $owner->zip_cd }}" class="form-control zip_code" placeholder="{{ config('constants.ZIPCOD_PLACEHOLDER') }}">
                    @if($errors->any() && $errors->has('zip_cd'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('zip_cd') }}</p>
                    @endif
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4"><span class="float-right"><b class="text-danger">*</b></span></label>
                <div class="col-md-5 col-8">
                    <input type="text" name="prefectures"
                        value="{{ old('prefectures') ?? $prefecture->prefectures_name }}"
                        class="form-control prefectures">
                    @if($errors->any() && $errors->has('prefectures'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('prefectures') }}</p>
                    @endif
                </div>
                <div class="div-hident col-4"></div>
                <div class="col-md-5 col-8 mt-2">
                    （都道府県）
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4"><span class="float-right"><b class="text-danger">*</b></span></label>
                <div class="col-md-5 col-8">
                    <input type="text" name="municipality_name"
                        value="{{ old('municipality_name') ?? $owner->municipality_name }}"
                        class="form-control municipality_name">
                    @if($errors->any() && $errors->has('municipality_name'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('municipality_name') }}</p>
                    @endif
                </div>
                <div class="div-hident col-4"></div>
                <div class="col-md-5 col-8 mt-2">
                    （市区町村郡）
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4"><span class="float-right"><b class="text-danger">*</b></span></label>
                <div class="col-md-5 col-8">
                    <input type="text" name="townname_address"
                        value="{{ old('townname_address') ?? $owner->townname_address }}"
                        class="form-control townname_address">
                    @if($errors->any() && $errors->has('townname_address'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('townname_address') }}</p>
                    @endif
                </div>
                <div class="div-hident col-4"></div>
                <div class="col-md-5 col-8 mt-2">
                    （町域名＋番地）
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4"></label>
                <div class="col-md-5 col-8">
                    <input type="text" name="building_name" value="{{ old('building_name') ?? $owner->building_name }}"
                        class="form-control">
                    @if($errors->any() && $errors->has('building_name'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('building_name') }}</p>
                    @endif
                </div>
                <div class="div-hident col-4"></div>
                <div class="col-md-5 col-8 mt-2">
                    （建物等）
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4 label-text font-weight-normal">電話番号 <b class="text-danger">*</b></label>
                <div class="col-md-5 col-8">
                    <input type="text" name="tel_no" value="{{ old('tel_no') ?? $owner->tel_no }}" placeholder="ハイフン無しで入力" class="form-control">
                    @if($errors->any() && $errors->has('tel_no'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('tel_no') }}</p>
                    @endif
                </div>
                <div class="col-md-5">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 col-4 label-text font-weight-normal">FAX番号</label>
                <div class="col-md-5 col-8">
                    <input type="text" name="fax_no" value="{{ old('fax_no') ?? $owner->fax_no }}" class="form-control">
                    @if($errors->any() && $errors->has('fax_no'))
                    <p class="text-danger error mb-2 mt-2">{{ $errors->first('fax_no') }}</p>
                    @endif
                </div>
                <div class="col-md-5">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-2 label-text col-4 pr-0 font-weight-normal pt-2">印鑑</label>
                <div class="text-center col-md-5">
                    <div class="d-flex justify-content-center align-items-center" id="div-input-image1" @if($owner->stamp)
                    style="display: none !important;"
                        @endif>
                        <label for="btnEditInputUpload1" id="file-label-images1" class="file-label-images1 file-label-images" style="max-width: 100% !important;">クリックまたはドラッグしてファイルを指定</label>
                        <input type="file" id="btnEditInputUpload1" name="stamp" class="file-input-images1 file-input-images" accept="image/*" value="" image="1"
                               style="width: 100% !important; cursor: pointer">
                    </div>
                    <div id="filePreviewEdit1" class="p-2 file-preview-area2 file-preview-area" @if($owner->stamp)
                    style="display: block"
                        @endif>
                        <img class="img-upload-preview1 img-preview" src="data:image/png;base64,{{ $owner->stamp }}" alt="preview uploaded file">
                        <svg width="2em" height="2em" viewBox="0 0 16 16" class="bi bi-x-circle-fill delete-btn" image="1" fill="currentColor">
                            <path fill-rule="evenodd" d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-4.146-3.146a.5.5 0 0 0-.708-.708L8 7.293 4.854 4.146a.5.5 0 1 0-.708.708L7.293 8l-3.147 3.146a.5.5 0 0 0 .708.708L8 8.707l3.146 3.147a.5.5 0 0 0 .708-.708L8.707 8l3.147-3.146z" />
                        </svg>
                        <div class="img-upload-name-preview1 img-upload-name-preview"></div>
                    </div>
                    @if($errors->any() && $errors->has('image.1'))
                        <p class="text-danger float-left mt-2">{{$errors->first('image.1')}}</p>
                    @endif
                </div>
            </div>
            <div class="form-group row">
                <div class="col-md-3 col-lg-3"></div>
                <div class="col-lg-2 mt-4 mr-3 ml-3">
                    <center>
                        <button id="showConfirm" class="btn btn-primary btn-submit-code w-100">
                            更新
                        </button>
                    </center>
                </div>
                <div class="col-md-1 col-lg-1"></div>
                <div class="col-lg-2 mt-4 mr-3 ml-3">
                    <center>
                        <a id="btn-cancel" href="javascript:void(0)" class="btn btn-secondary w-100" data-toggle="modal"
                           data-target="#modalCancel">キャンセル</a>
                    </center>
                </div>
                <div class="col-md-4 col-lg-4"></div>
            </div>
        </form>
    </div>
    <!-- /.card-body -->
</div>
@include('owner::layouts.modal_confirm')
@include('owner::layouts.modal_cancel')
@endsection
@section('js')
<script>
    var key = '{{ config('constants.KEY_GEOCODING_API') }}';
    var messageRequired = '{{ __('message.member.required') }}';
    var checkZipCode = true;
    var messageConfirmRequired = "{{ __('message.member.confirm_required') }}"
</script>
<script src="{{ mix('/js/find_address.js') }}"></script>
<script src="{{ mix('/js/register_member.js') }}"></script>
<script src="{{ mix('/js/validate_owner_edit.js') }}"></script>
<script src="{{ mix('/js/moment.min.js') }}"></script>
<script src="{{ mix('/js/common/common-image-video-upload.js') }}"></script>
@endsection
